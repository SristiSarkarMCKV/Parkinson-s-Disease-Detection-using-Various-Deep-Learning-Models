...
# Professional Medical Theme Background (Clean clinical gradient fallback)
...
# ---------------------------------------------------------
# Robust AI Model Engine with Enhanced Training Architecture
# ---------------------------------------------------------
@st.cache_resource(show_spinner=False)
def get_trained_model():
    model_path = 'PD_CNN_Clinical_Model.h5'
    
    # Force retrain or build if not found
    if os.path.exists(model_path):
        try:
            return tf.keras.models.load_model(model_path)
        except Exception:
            pass # Rebuild if corrupted cache file
    ...
    if os.path.exists(local_zip_filename) and not os.path.exists(os.path.join(extract_path, 'ParkinsonDisease')):
        with zipfile.ZipFile(local_zip_filename, 'r') as zip_ref:
            zip_ref.extractall(extract_path)

    # Locate training directory robustly
    train_dir = None
    for root, dirs, files in os.walk(extract_path):
        if 'TRAIN' in dirs or 'train' in dirs:
            train_dir = os.path.join(root, 'TRAIN' if 'TRAIN' in dirs else 'train')
            break
            
    if not train_dir:
        # Fallback path structure match
        possible_path = os.path.join(extract_path, 'ParkinsonDisease', 'ParkinsonDisease', 'TRAIN')
    ...
    # Automatically identify alphabetical label mapping to ensure exact ground truth correlation
    # train_generator.class_indices typically sorts folder names alphabetically e.g., {'Affected': 0, 'Not affected': 1}
    st.session_state['class_indices'] = train_generator.class_indices

    # Enhanced Architecture with BatchNormalization for higher clinical accuracy & stability
    model = Sequential([
    ...
    # Advanced Callbacks for optimized precision & stopping convergence
    lr_reduction = ReduceLROnPlateau(monitor='val_loss', patience=2, factor=0.5, min_lr=1e-6, verbose=0)
    early_stopping = EarlyStopping(monitor='val_accuracy', patience=4, restore_best_weights=True, verbose=0)
...
with st.spinner("Initializing Clinical Diagnostic Model & Verifying Weights..."):
...
def classify_image(image):
    ...
    # Resolve class mapping logic dynamically based on folder generation indexes
    class_indices = st.session_state.get('class_indices', {})
    affected_idx = 0
    for k, v in class_indices.items():
        if 'affect' in k.lower() and 'not' not in k.lower():
            affected_idx = v
            break

    # If Affected is label index 0, model output near 0 means Affected. If index 1, output near 1 means Affected.
    if affected_idx == 0:
        is_affected = prediction < 0.5
        affected_prob = (1.0 - prediction) * 100
        normal_prob = prediction * 100
    else:
        is_affected = prediction >= 0.5
